...
for i=1:miter
    ...
    [tour] = antTour(city, m, n, h, t, alpha, beta,sales,depot);
...
for i=1:n+sales-1
    pathOfmTSP(index, i-j) = generalPath(1,i);
...
pathOfmTSP = horzcat(pathOfmTSP,pathOfmTSP(:,1))

pathObtained = fix((n-1)/sales)+2;
for i=1:sales
    for j=1:pathObtained
        X(j) = x(pathOfmTSP(i,j));
        Y(j) = y(pathOfmTSP(i,j));
        index = index + 1;
    end
