...
for i=2:sizeOfOptimalPath(1,1)
    currentX = test_x1(length(test_x1),1);
    currentY = test_x1(length(test_x1),2);
    startPoint1 = [currentX;currentY;atan2(optimalPath(i,2)+.5-currentY,optimalPath(i,1)+.5-currentX)];
    targetPoint1(1,1) = optimalPath(i,1)+.5;
    targetPoint1(1,2) = optimalPath(i,2)+.5;
    sim("formationControl.slx");
    leaderPath = cat(1,leaderPath,test_x1);
end
plot(leaderPath(:,1),leaderPath(:,2),'bo');