...
binMap = binaryOccupancyMap(imageBW, 100);
% subplot(2,2,2);
% show(binMap);
% grid on;
% hold on;
...
for i=1:miter
    ...
    disp(['Path Optimize: ',num2str(i),'%    Current Distance: ',num2str(min(distance))]);
    pause(.1);
    for temp=1:n
...
initX1 = optimalPath(1,1)+5;
initY1 = optimalPath(1,2);
v1_init = 0;
a1_init = 0;
w1_init = 0;
alp1_init = 0;
...
startPoint1 = [initX1;initY1;atan2(optimalPath(1,2)+.5-initY1,optimalPath(1,1)+.5-initX1)];
...
for i=2:sizeOfOptimalPath(1,1)
    ...
    startPoint1 = [currentX1;currentY1;atan2(optimalPath(i,2)+.5-currentY1,optimalPath(i,1)+.5-currentX1)];
    ...
    follower2Path = cat(3,follower2Path,test_x2);
    follower3Path = cat(3,follower3Path,test_x3);
    disp(['Formating Calculation: ',num2str(i*100/sizeOfOptimalPath(1,1)),'%']);
end
%% Convert data
