...
optimalPathSline = [xx;yy];
optimalPathSline = optimalPathSline.';
% figure;
% plot(x,y,'ro');
% hold on;
...
% optimalPathSline2(1,:)=optimalPathSline(1,:);
% k=2;
% for i=2:sizeOfoptimalPathSline(1,1)
%     if (optimalPathSline(i,1) ~= optimalPathSline(i-1,1))&&(optimalPathSline(i,2) ~= optimalPathSline(i-1,2))
%         optimalPathSline2(k,:)=optimalPathSline(i,:);
%         k=k+1;
%     end
% end
%% Initial parameter
...
% invPathTemp = [];
% for i=1:n
%     pathTemp = doptimalPathSline{besttour(l,i),besttour(l,i+1)};
%     for j=length(pathTemp):-1:1
%         invPathTemp(length(pathTemp)-j+1,:) = pathTemp(j,:);
%     end
%     optimalPathSline = cat(1,optimalPathSline,invPathTemp);
% end
% optimalPathSline = PathSmoothing(optimalPathSline);
sizeOfoptimalPathSline = size(optimalPathSline);
initX1 = optimalPathSline(1,1)+.5;
initY1 = optimalPathSline(1,2)+.5;
v1_init = 0;
a1_init = 0;
w1_init = 0;
alp1_init = 0;
...
initX3 = initX1-5;
initY3 = initY1+5;
targetPoint1 = [optimalPathSline(1,1)+.5,optimalPathSline(1,2)+.5];


%%%
theta_x1 = atan2(optimalPathSline(2,2)+.5-initY1,optimalPathSline(2,1)+.5-initX1);
...
t=(0:1:sizeOfoptimalPathSline(1,1)-1);
vx = gradient(optimalPathSline(:,1),t);
vy = gradient(optimalPathSline(:,2),t);
v = sqrt(vx.^2+vy.^2);

t_temp = t.';
dx = gradient(optimalPathSline(:,1));
dy = gradient(optimalPathSline(:,2));
theta = atan2(dy,dx);
...
for i=2:sizeOfoptimalPathSline(1,1)
    ...
    %
    %%%
theta_x1 = atan2(optimalPathSline(i,2)+.5-currentY1,optimalPathSline(i,1)+.5-currentX1);
if theta_x1<0 %khi mega <0 se xay ra hien tuong xoay tron
    k13=fix(-theta_x1/(2*pi));
    theta_x1 = 2*(k13+1)*pi +theta_x1;
end
...
%     startPoint3 = [currentX3;currentY3;currentTeta3];
    startPoint2 = [currentX2;currentY2;theta_x1];
    startPoint3 = [currentX3;currentY3;theta_x1];
    targetPoint1(1,1) = optimalPathSline(i,1)+.5;
    targetPoint1(1,2) = optimalPathSline(i,2)+.5;
    test_x1 = [];
    test_x2 = [];
    test_x3 = [];
    sim("formationControl.slx");
    ...
    follower2Path = cat(3,follower2Path,test_x2);
    follower3Path = cat(3,follower3Path,test_x3);
    disp(['Formating Calculation: ',num2str(i*100/sizeOfoptimalPathSline(1,1)),'% Target: ',num2str(targetPoint1(1,:)), ' Start: ',num2str(startPoint1temp(1,:))]);
end
%% Convert data
...
%% Animation
j=size(leaderPath,1);
plot(leaderPath(:,1),leaderPath(:,2),'g-','LineWidth',1.5);
hold on;
x1 = plot(leaderPath(1,1),leaderPath(1,2),'go');
hold on;
plot(follower2PathNew(:,1),follower2PathNew(:,2),'c-','LineWidth',1.5);
hold on;
x2 = plot(follower2PathNew(1,1),follower2PathNew(1,2),'co');
hold on;
plot(follower3PathNew(:,1),follower3PathNew(:,2),'m-','LineWidth',1.5);
hold on;
x3 = plot(follower3PathNew(1,1),follower3PathNew(1,2),'mo');
hold on;
qw{1} = plot(nan, 'g-','LineWidth',1.5);
qw{2} = plot(nan, 'c-','LineWidth',1.5);
qw{3} = plot(nan, 'm-','LineWidth',1.5);
legend([qw{:}], {'Leader','Follower 1','Follower 2'}, 'location', 'northeast')
